...
class REST_Controller extends CI_Controller {
	...
	public function __construct()
	{
		...
		switch ($this->request->method)
		{
			...
			case 'delete':
				...
				parse_str(parse_url($_SERVER['REQUEST_URI'], PHP_URL_QUERY), $this->_delete_args);
				break;
		}
		...
		// Check for keys, if a valid key is found, skip further authentication
		...
		if ( $this->auth_override !== TRUE && $this->_allow !== TRUE)
		{
	...
	*/
	...
	protected function _detect_api_key()
	{
		...
		log_message('debug', 'Detect API Key: key_variable is ' . $api_key_variable);
		...
		if ($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name))
		{
			...
			log_message('debug', 'Detect API Key: successfully authenticated');
			return TRUE;
		}

		// No key has been sent
		log_message('debug', 'Detect API Key: failure, no key found');
		return FALSE;
	}
	...
	*/
	...
	protected function _check_limit($controller_method)
	{
		...
		if (!empty($this->rest->ignore_limits) OR !isset($this->methods[$controller_method]['limit']))
		{
			...
			log_message('debug', 'Rate Limiting: ignore limits is set or there is no limit for this controller');
			return TRUE;
		}
		...
		{
			log_message('debug', 'Rate Limiting: no requests from this key in this hour');
		...
		else
		{
			...
			{
				log_message('debug', 'Rate Limiting: Too many requests this hour. Count: '. $result->count);
				return FALSE;
			}
		...
		log_message('debug', 'Rate Limiting: Limit not reached.');
		return TRUE;
	}
	/*
	 * Auth override check
	...
	*/
	...
	{
		log_message('debug', 'Check Login: ' . $username);
		if (empty($username))
		{
			log_message('debug', 'Check Login: failure empty username');
			return FALSE;
		}

		if (strtolower($this->config->item('auth_source')) == 'ldap')
		{
		...
		{
			log_message('debug', 'Check Login: failure, invalid username');
			return FALSE;
		}
		...
		{
			log_message('debug', 'Check Login: failure, wrong password');
			return FALSE;
		}
		log_message('debug', 'Check Login: Success');
		return TRUE;
	}
	...
	protected function _prepare_digest_auth()
	{
		...
		/* The $_SESSION['error_prompted'] variable is used to ask
